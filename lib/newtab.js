...
const drag = (ev) => {
  ...
  ev.dataTransfer.setData("item-title", ev.target.innerHTML);
...
const drop = (ev) => {
  ...
  console.log(ev.dataTransfer.getData("item-id"));
  console.log(url);
  console.log(title);
...
const loadOpenTabs = () => {
  chrome.tabs.query({}, (tabs) => {
    ...
    openTabs.forEach( tab => {
      ...
      $('.open-tabs').append(`<li class=single-open-tab grabbable draggable=true id=${tab.id} data-url=${tab.url} data-title=${tab.title}>${tab.title}</li>`);
  ...
  chrome.storage.sync.get(ob => {
    ...
    if (allSavedTabs['tabs4later']) {
      ...
      allSavedTabsKeys.forEach(key => {
        ...
        // $(`#${id}`).removeClass('single-open-tab');
        // $(`#${id}`).addClass('single-groupped-tab');
        $('#group1').append(`<li class=single-groupped-tab data-id=${key} data-url=${url} data-title=${title}>${title}</li>`);
      });
      $( ".single-groupped-tab" ).click((e) => {
        console.log( "Handler for .click() called." );
        // console.log(e.target.attributes);
        chrome.tabs.create({url: e.target.attributes[2].value, active: true}, updateOpenTabs);
      });
    }
  });
};
